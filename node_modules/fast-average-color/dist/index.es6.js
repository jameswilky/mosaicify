...
class FastAverageColor {
  ...
  getColorAsync(resource, options) {
    if (!resource) {
      return Promise.reject(
        Error("Call .getColorAsync(null) without resource.")
      );
  ...
  getColor(resource, options) {
    ...
    if (
      ...
      this._outputError(
        options,
        `incorrect sizes for resource "${resource.src}".`
      );
    ...
    try {
      ...
      const bitmapData = this._ctx.getImageData(
        0,
        0,
        size.destWidth,
        size.destHeight
      ...
      this._outputError(
        options,
        `security error (CORS) for resource ${
          resource.src
  ...
  getColorFromArray4(arr, options) {
    ...
    switch (options.algorithm || "sqrt") {
      ...
      default:
        throw new Error(
          `${ERROR_PREFIX}${options.algorithm} is unknown algorithm.`
        );
    }
    ...
    return new Promise((resolve, reject) => {
      const onload = () => {
        ...
        onerror = () => {
          ...
          reject(
            new Error(`${ERROR_PREFIX}Error loading image ${resource.src}.`)
          );
        },
        onabort = () => {
          unbindEvents();
          ...
          reject(
            new Error(`${ERROR_PREFIX}Image "${resource.src}" loading aborted.`)
          );
        },
        unbindEvents = () => {
